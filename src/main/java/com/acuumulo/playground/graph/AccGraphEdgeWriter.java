...
public class AccGraphEdgeWriter implements AccConstants, GraphConstants{
	 ...
	 * @param edgeAttrs Map<String,String> 
	 * @param auths String	
	 * 
	...
	public static void writeEdge(
		...
		writeEdge(writer,eri.getRowId(),nodes[0],nodes[1],skipStandard,skipSort,edgeType,edgeAttrs,auths);
	}
	...
	public static void writeEdge(
		...
		writeEdge(writer,nA,nB,true,edgeType,edgeAttrs,auths);
	}
	...
	public static void writeEdge(
		...
		String rid = EdgeRowId.assembleEdgeRowId(nA, nB, true, true);
		
		//call to most efficient method, direct to skip checks.
		writeEdge(writer,rid,nodes[0],nodes[1],true,true,edgeType,edgeAttrs,auths);
	}
	 ...
	 * @param nodeRowIdA String
	 * @param nodeRowIdB String
	 * @param skipSortEmptyChecks boolean
	 * @param edgeType String
	 * @param edgeAttrs Map<String,String> 
	 * @param auths String
	 * 
	...
	public static void writeEdge(
		...
		if (edgeAttrs == null) return;
		
		for (Entry<String,String> entry : edgeAttrs.entrySet()){
			
			String k = AccUtils.manipulate(entry.getKey(), ManOp.EMPTY_TO_NULL);
			String v = AccUtils.manipulate(entry.getValue(), ManOp.EMPTY_TO_NULL);
